...
- Per user: every account enrols with its own secret and its own backup codes
...
- Grace period: a new user gets a set number of days to enrol before setup becomes mandatory
...
- Per-user admin actions on the settings page: one-time setup link, fresh backup codes, reset
...
- Every success and failure lands in the syslog with the user and the source IP
